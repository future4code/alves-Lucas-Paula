...
const getAddressInfo = async (cep: string, bairroPessoa: string, numero: string, compemento: string): Promise<string> => {
    try {
        ...
        const address = `${logradouro ? logradouro : `${numero}, ${compemento}`}, ${bairro ? bairro : bairroPessoa} - ${localidade} / ${uf}`
        // console.log(address)
        return address
    } catch (error) {
        throw new Error();
